  ...
  <article>
    <div class="toggles">
      <input
        ...
        id="checkbox"
        class="ios-toggle"
        v-model="photoshopedComputer"
      />
      <label
        for="checkbox"
        class="checkbox-label"
        data-off="No Photoshop"
        data-on="Photoshoped"
      ></label>
    </div>
    <ApolloQuery
      :query="require('../graphql/getMemes.gql')"
      :variables="{ photoshoped }"
    >
      <template slot-scope="{ result: { loading, error, data }, isLoading }">
        ...
        <div v-else-if="data && shuffle(data.memes)">
          <article
            v-for="meme in data.memes"
            :key="meme.id"
            class="fl w-50 w-33-l link overflow-hidden relative meme sans-serif"
          >
